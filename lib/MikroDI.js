...
class MikroDI {
  ...
  build() {
    ...
    const writer = new ContextWriter(graph, this.options.baseDir, this.options.contextDir);
    writer.save(file);
  ...
  _discover(path, graph) {
    ...
    files.forEach(file => {
      ...
      const parser = new ServiceParser(this.options.baseDir, this.options.services);
