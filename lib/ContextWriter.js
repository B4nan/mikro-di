...
class ContextWriter {
  ...
  _processService(className) {
    ...
    let ret = '';
    ret += `  /**\n`;
    ret += `   * @returns {${node.name || className}}\n`;
    ret += `   */\n`;
    ret += `  get ${className}() {\n`;
    ret += `    if (!this._map.${className}) {\n`;
    ret += `      const ${node.name || className} = require('${node.path}');\n`;
    ...
    if (node.classSyntax) {
      ret += `      this._map.${className} = new ${node.name || className}(${dependencies});\n`;
    } else {
      ret += `      ${className}.constructor(${dependencies});\n`;
      ret += `      this._map.${className} = ${node.name || className};\n`;
    }
